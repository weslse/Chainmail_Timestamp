...
void setVolume()
{
	...
	inStream.read(reinterpret_cast<char*>(volume), VOLUME_SIZE);
	...
	for (int y = 0; y < ARR_HEIGHT; ++y)
		for (int x = 0; x < ARR_WIDTH; ++x)
			volumeSlice[y][x] = volume[140][y * 4][x * 4]; // 64
			//volumeSlice[y][x] = volume[130][y][x]; // 256
			//volumeSlice[y][x] = volume[110][y*2][x*2]; // 128
}
